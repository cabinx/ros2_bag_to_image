#include "ros2_bag_to_image/bag_to_image.hpp"

BagToImage::BagToImage(const rclcpp::NodeOptions &options) :
  Node("bag_to_image", options) {
  // IO
  std::vector<std::string> input_topics_init_;
  ...
  input_topics_ = this->declare_parameter<std::vector<std::string>>("input/topics", input_topics_init_);
